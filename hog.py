...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    # BEGIN PROBLEM 7
    def say(score0, score1,high=previous_high,score=previous_score):
        if who == 0:
            diff = score0 - score
            if score0 > score and diff > high:
                if diff == 1:
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    while num_roll <= 10:
        ...
        avg = averaged_roll_dice(num_roll, dice)
        if avg > max_avg:
            max_roll = num_roll
            max_avg = avg
        num_roll += 1    
#        elif avg == max_avg:
#            max_roll = min (max_roll, num_roll)
    return max_roll
    # END PROBLEM 9
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    if new_score < opponent_score and is_swap(new_score, opponent_score) or free_bacon(opponent_score) >= margin:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
